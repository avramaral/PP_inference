...
```{r}
(lambda_hat <- length(pp) / 100)
```

### Non-homogeneous Poisson process

To simulate from a non-homogeneous Poisson process, we can use the thinning ([Lewis and Shedler, 1979](https://onlinelibrary.wiley.com/doi/10.1002/nav.3800260304)).
...
For a Poisson process, we have that $\mathcal{N}(D) \sim \text{Poisson}(\int_D\lambda(x)dx)$ and, for each $x$, $\mathbb{P}(x_i) = \lambda(x_i) / \frac{\int_D\lambda(x)dx}. Therefore,
\begin{align*}
